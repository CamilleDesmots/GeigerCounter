...
@Singleton
@ApplicationScoped
public class DataProvider13 implements Serializable {

    private static final Logger LOGGER = Logger.getLogger(DataProvider13.class.getName());
    // For grouping by hour
    private static final DateTimeFormatter DATETIMEFORMATTER1 = DateTimeFormatter.ofPattern("yyyy-MM-dd HH");

    private Map <LocalDateTime, DoubleSummaryStatistics> cpmStatisticsPerHour;
    ...
    public DataProvider13() {
        ...
        this.cpmStatisticsPerHour = new HashMap <>();
    }
    ...
    private void update() {
        ...
        newDateEnd = LocalDateTime.now().truncatedTo(ChronoUnit.MINUTES);
        ...
        if (newDateEnd.minusMinutes(1).isAfter(newDateBegin)) {
            ...
            if (newListCpm.isEmpty() == false) {
                ...
                this.cpmStatisticsPerHour.putAll(summaryStatisticsByHour);
                // LOGGER.log(Level.FINEST, "After this.cpmStatisticsPerHour.putAll(summaryStatisticsByHour).");
            }
            ...
            boolean removed;
            removed = this.cpmStatisticsPerHour
                    .keySet().removeIf(p -> {
                        return p.isBefore(this.dateBegin);
                    });

        }
        // LOGGER.log(Level.FINEST, "The list has {2} records for the period [{0}...{1}] {2}", new Object[]{this.dateBegin, this.dateEnd, this.cpmStatisticsPerHourAverage.keySet().stream().count()});
    }
     ...
     */
    public Map <LocalDateTime, DoubleSummaryStatistics> getStatisticsPerHour(){
        this.update();
        // LOGGER.log(Level.FINEST, "Number of elements in this.cpmStatisticsPerHour {0}.", new Object[]{this.cpmStatisticsPerHour.size()});
        return this.cpmStatisticsPerHour;
    }
}